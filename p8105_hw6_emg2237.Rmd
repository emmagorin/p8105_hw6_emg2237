...
birthweight_df =
  ...
  select(baby_weight_g, baby_length_cm, ga_weeks, baby_head_cm, baby_sex) # to add: anything I use for a model
```
...
cv_df = 
  ...
  mutate(
    ...
    #rmse_my_mod = map2_dbl(.x = wiggly_mod, .y = test, ~rmse(model = .x, data = .y))
  )
```
